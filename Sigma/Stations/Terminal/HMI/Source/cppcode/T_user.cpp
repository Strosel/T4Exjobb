...
void resetwindow_ready(_MY_PICTURE *mp)
{
  ...
  if(mp->JustProgram == _TRUE)
  {
    ...
    pos_2button(&room, &mp->Room, _FALSE);
    OutMultiTextAlign((void*)"<ESC>...cancle", 0, &room, fnt, T_MIDBOUND|T_CENTERBOUND|T_COPY|T_SOLID|T_PROP, FRAME_PLASTIC, WHITE, MakeColor_Makro(LIGHTRED, LIGHTRED), MakeColor_Makro(WHITE, RED), sizeof(_ASCII));
   #else
